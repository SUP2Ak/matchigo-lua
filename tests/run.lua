...
test("P.float", function()
    assertTrue(m.isMatching(P.float, 1.5))
    assertFalse(m.isMatching(P.float, 5))
    assertFalse(m.isMatching(P.float, "x"))
end)
...
test("P.shape strict — exact keys", function()
    local pat = P.shape({ kind = "click", x = P.number })
    assertTrue(m.isMatching(pat, { kind = "click", x = 5 }))
end)
test("P.shape strict — rejects extras", function()
    local pat = P.shape({ kind = "click", x = P.number })
    assertFalse(m.isMatching(pat, { kind = "click", x = 5, extra = "no" }))
end)
test("P.shape strict — rejects missing required", function()
    local pat = P.shape({ kind = "click", x = P.number })
    assertFalse(m.isMatching(pat, { kind = "click" }))
end)
test("P.shape strict — rejects non-table", function()
    local pat = P.shape({ kind = "click" })
    assertFalse(m.isMatching(pat, "click"))
    assertFalse(m.isMatching(pat, nil))
    assertFalse(m.isMatching(pat, 42))
end)
test("P.shape strict — empty shape only matches empty table", function()
    local pat = P.shape({})
    assertTrue(m.isMatching(pat, {}))
    assertFalse(m.isMatching(pat, { x = 1 }))
end)
test("P.shape strict — optional field accepts absent", function()
    local pat = P.shape({ id = P.number, name = P.optional(P.string) })
    assertTrue(m.isMatching(pat, { id = 1 }))
    assertTrue(m.isMatching(pat, { id = 1, name = "Bob" }))
    assertFalse(m.isMatching(pat, { id = 1, name = "Bob", age = 30 }))
end)
test("P.shape strict — non-table arg throws at construction", function()
    assertThrows(function() P.shape("nope") end)
    assertThrows(function() P.shape(42) end)
end)
test("P.shape strict — bindings via P.select", function()
    local rules = {
        { with = P.shape({ kind = "click", x = P.select("x"), y = P.select("y") }),
          handler = function(sel) return sel.x + sel.y end },
        { otherwise = function() return -1 end },
    }
    assertEq(m.match({ kind = "click", x = 3, y = 4 }, rules), 7)
    -- extra key blocks the strict shape, falls through to otherwise
    assertEq(m.match({ kind = "click", x = 3, y = 4, z = 0 }, rules), -1)
end)
test("P.shape strict — nested strict shape", function()
    local pat = P.shape({
        user = P.shape({ id = P.number, name = P.string }),
    })
    assertTrue(m.isMatching(pat, { user = { id = 1, name = "Bob" } }))
    assertFalse(m.isMatching(pat, { user = { id = 1, name = "Bob", role = "admin" } }))
    assertFalse(m.isMatching(pat, { user = { id = 1, name = "Bob" }, extra = 1 }))
end)
