...
return function(env)
    ...
    -- ── strict shape : `{| ... |}` ───────────────────────────────────────
    test("lex : strict-shape delimiters", function()
        assertEq(kinds("{| |}"), "LBRACE_PIPE,PIPE_RBRACE,EOF")
    end)
    test("lex : strict-shape with content", function()
        assertEq(kinds("{|x:Num|}"),
            "LBRACE_PIPE,IDENT_LO,COLON,IDENT_UP,PIPE_RBRACE,EOF")
    end)
    test("parse : strict shape — empty", function()
        local r = parse("{||}")
        assertEq(r.kind, ast.SHAPE); assertEq(r.strict, true); assertEq(#r.fields, 0)
    end)
    test("parse : strict shape — typed field", function()
        local r = parse("{| kind: 'click', x: Num |}")
        assertEq(r.kind, ast.SHAPE); assertEq(r.strict, true); assertEq(#r.fields, 2)
    end)
    test("parse : strict shape — shorthand binding", function()
        local r = parse("{| x, y: Num |}")
        assertEq(r.strict, true)
        assertEq(r.fields[1].shorthand, true)
    end)
    test("parse : strict shape — rejects ...rest", function()
        assertThrows(function() parse("{| a: Str, ...rest |}") end)
    end)
    test("parse : non-strict shape preserves strict=false", function()
        local r = parse("{ k: 'click' }")
        assertEq(r.strict, false)
    end)
    test("parse : strict shape inside union", function()
        local r = parse("{| kind: 'click' |} | {| kind: 'key' |}")
        assertEq(r.kind, ast.UNION); assertEq(#r.items, 2)
        assertEq(r.items[1].strict, true); assertEq(r.items[2].strict, true)
    end)
